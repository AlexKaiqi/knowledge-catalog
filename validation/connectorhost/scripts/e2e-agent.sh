...
user_repo="${evidence_dir}/user-repo"
...
mkdir -p "${user_repo}" "${kc_home}" "${host_home}"
cp -R "${host_src}/testdata/agent-repo/." "${user_repo}/"
cp "${host_src}/README.md" "${user_repo}/CONNECTOR_SPEC.md"
cp "${host_src}/AGENT_TASK.md" "${user_repo}/TASK.md"
...
"${host_bin}" --home "${host_home}" mount \
  --repo "${user_repo}" --kc-url "${kc_url}" --as connector/service-observer \
  >"${evidence_dir}/mount.json"
...
python3 - "${user_repo}/sources/services.json" <<'PY'
import json, sys
path = sys.argv[1]
data = {
