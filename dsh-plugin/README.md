...
Agent 还会加载两级开发契约：`integration-development` 约束 Collector 与可选实时资源访问的通用运行边界；`connector-development` 负责在场景的公共 Connector 开发仓中生成一目录一个业务接入、测试并交给 Host validate/preview。模型不需要从 README 猜工作流。
...
src/
  ...
  skill.ts    在 ctx.skills 注册随包的 Catalog、Integration 与 Connector Skills
...
skills/connector-development/     公共 Connector 仓的对话开发流程
