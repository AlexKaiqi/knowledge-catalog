...
deployment_config="$(go -C "$root_dir" run ./scripts/fixture-deployment --root "$kc_home" --catalog kr://scene/catalog --principal "$admin_principal" --repo "kr://scene/knowledge=$kc_home/source")"
...
"$kc_bin" catalog repo attach --server "$server_url" --as "$admin_principal" --repo kr://scene/knowledge >/dev/null
