...
定义运行系统如何发现故障、解释性能、衡量可靠性，并把一次请求关联到已有的知识访问证据。

## Non-Goals

- 不把 metric、diagnostic log 或 distributed trace 写成知识（文首）。
- 不代替版本化访问证据（`OBSERVABILITY.md`）。
- 不改变 ⓪–③。不以面板数量为完成标准。
...
`principal` 和 `onBehalfOf` 只有在认证边界已建立、委托已验证时才可用于安全结论。
