...
证据来自 `.data/scenes/` 的 construct/probe、节点点名的 go-test，以及叶子 help 第一句。没有三人组，对不能判过。
...
场景名是 `.data/scenes/` 节点，不是另一份命令树。一条命令挂在它被钉住的状态上；前态只提供构建前置。
...
场景：`catalog-initialized` → `http-served` → `catalog-allow-ready` → `catalog-read-granted` / `grants-bootstrapped`。
...
| `show` | `catalog-initialized` / `catalog-inventory-visible` | 当前 Catalog 挂了哪些仓和知识集 | 过 | 过：`catalog list`、`schema list`（实体不是库存）、对象 LIST（已否决） | 过：`{catalogId, repositories[].id, datasets}`；`home`/`title`/`summary` 缺席。help「显示当前 Catalog 的 Repository 与知识集」 | 过 | 过：`catalog.read` 不放行正文 | 日常 | 成立 |

`absent-product-surfaces` 钉住：没有对象 LIST、checkout/export、MCP 正路径。缺的不是本表漏判。

---

## 2. 挂源给权 — write / compose 供给

场景：`managed-repository-created`、`repository-attached`、`catalog-inventory-visible`、`grant-revoked`、`repository-archived`。

| 命令 | 场景 | 一句语义 | 问 | 刀（对手） | 对 | 形 | 侧 | 路 | 成立 |
|---|---|---|---|---|---|---|---|---|---|
| `create --name` | `managed-repository-created` | 向平台要一个空仓 | 过 | 过：`git init`、直接改 Catalog 文件、HTTP POST 集合 | 过：`{catalog, repositoryId, status:APPLIED, head}`；无 `--name`/`--url` → `USAGE_INVALID`。help「创建托管 Repository 或连接自有 Repository」 | 过 | 过：不 attach、不发 `knowledge.read` | 日常 | 成立 |
| `create --url` | `managed-repository-created` | 把已有仓接到 KC 能打开 | 过 | 过：`attach`（登记）、裸 clone | 过：与 `--name` 互斥；缺 `--credential-file` → `USAGE_INVALID` | 过 | 过：连接 ≠ 挂进 Catalog | 日常 | 成立 |
...
| `detach` | `repository-archived` | 从当前 Catalog 拿下这个仓 | 过 | 过：`catalog archive`（整间房）、删 Snapshot | 过：`{repositoryId, detached:true}`；`writer head` 仍有 commit。help「从当前 Catalog 拿下 Repository」 | 过 | 过：不删对象 | 日常 | 成立 |
| `grant add` | 各 `*-granted` construct | 允许某人在这仓、这间 Catalog 或这个 Dataset 做一类动作 | 过 | 过：改 `allow.json`、HTTP POST shares | 过：`{id, principal, catalog\|repository, actions}`。help「发放一条仓、Catalog 或 Dataset 范围的授权规则」 | 过 | 过：`--repo` 与 `--catalog` 二选一；Dataset 消费权加 `--dataset`；不能发 `*` | 日常 | 成立 |
...
| `grant remove` | `grant-revoked` | 撤销这一条授权规则 | 过 | 过：`grant add`、`detach` | 过：`{revoked:alw_1}`，随后 `rules:[]`。help「撤销一条授权规则」 | 过 | 过：立即生效，旧 pin 不能绕过（KS-02） | 日常 | 成立 |

---

## 3. 读知识 — consume 后半

场景：`system-schema-published` → `schema-read-granted` → `projection-synced` → `knowledge-search-granted` → `knowledge-read-granted`；对象史在 `knowledge-published`。

| 命令 | 场景 | 一句语义 | 问 | 刀（对手） | 对 | 形 | 侧 | 路 | 成立 |
|---|---|---|---|---|---|---|---|---|---|
| `schema list` | `system-schema-published` / `schema-read-granted` | 这仓发布了哪些实体类型 | 过 | 过：`show`、空 `search`、对象 LIST、`read` 合同正文 | 过：`{repository, commit?, schemas[].objectId, entity, description}`。无 `--repo` → `USAGE_INVALID`。help「列出一个 Repository 已发布的实体，不是对象目录或合同正文」 | 过 | 过：`catalog.read` 不隐含 | 日常 | 成立 |
...
| `read` | `knowledge-read-granted` / `knowledge-published` | 打开这一份知识的正文 | 过 | 过：`search`、`log`/`provenance`、Linux `cat`、`writer head` | 过：`{repository, objectId, commit, value}`。help「读取一个知识对象在固定 commit 上的正文」 | 过 | 过 | 日常 | 成立 |
| `resolve` | `knowledge-published` / `principals-granted` | 这一份知识在不在、钉在哪一版 | 过 | 过：`read`（打开正文）、`writer head`（仓 HEAD） | 过：`{repository, objectId, commit, status}`，`address` 缺席。help「检查对象是否存在并给出解析到的 commit，不打开正文」 | 过 | 过：缺对象是 `UNRESOLVED` 成功回执（与 HTTP RESOLVE 对齐，不是 `read` 的错误码） | 进阶 | 成立 |
...
| `binding show` | `access-handle-published` | 这个字段声明怎么连墙外 | 过 | 过：`read`（句柄正文）、`access`（观察值） | 过：Binding 声明。help「查看 Binding 声明」 | 过 | 过：只看声明 | 进阶 | 成立 |
| `access` | `access-handle-published` / `resource-access-granted` | 读墙外此刻的观察值 | 过 | 过：`binding show`、`read`、`invoke` | 过：Schema 无 origin → `CAPABILITY_UNSATISFIED`；成功 `{objectId, aspectName, schemaRef, value, basis}`。help「按 Schema origin 与实体 ID 读取该 Aspect 的当前观察」 | 过 | 过：`--operation` 拒，指向 `invoke` | 进阶 | 成立 |
| `invoke` | `access-handle-published` | 对墙外执行一次已声明的操作 | 过 | 过：`access` | 过：描述无 origin → `CAPABILITY_UNSATISFIED`。help「调用 ResourceDescriptor 操作」 | 过 | 过：仓读权不能替代源系统强制 | 进阶 | 成立 |
...
场景：`domain-schema-published`；实例 `knowledge-published` / `semantic-knowledge-constructed`；`writer-granted`。
...
场景：`knowledge-set-defined`、`dataset-retired`、`dataset-resolve-granted`、`principals-granted`、`file-view-planned`。
...
| `dataset retire` | `dataset-retired` | 退役这个配方 | 过 | 过：`detach`、删仓 | 过：`{dataset, retired:true}`，`show` 仍见 id 且 `retired:true`。help「退役命名配方」 | 过 | 过：不删成员仓 | 进阶 | 成立 |
| `dataset overlay` | `file-view-planned` / `knowledge-set-defined` probe | 只在本机合成一份不发表的配方 | 过 | 过：`dataset define`（发表） | 过：`{setId, sources[]}`；`show` 的共享 Dataset 不变；用 overlay 名去 `read --dataset` → `KNOWLEDGE_SET_INVALID`。缺操作数 → `USAGE_INVALID`。help「只在本机合成一份不发表的配方」 | 过：`--file` 底稿 + `--overlay` 补丁 | 过：不改共享定义 | 进阶 | 成立 |

---

## 6. 合入治理

场景：`proposal-opened` → `proposal-previewed` → `proposal-validated` → `validation-recorded` → `proposal-merged`。进阶。
...
| `governance preview create` | `proposal-previewed` | 把 proposal 叠到本次解析的 Dataset 上，得到可校验的 Preview 坐标 | 过 | 过：`dataset define` | 过：`{previewId, candidate.commitId, candidate.repositoryId}`；随后 `read --repo` 仍旧值。help「给出 candidate 坐标，不改 main」 | 过：必须 `--dataset` | 过：main 仍不动；正文走 `read --commit` | 进阶 | 成立 |
| `governance preview validate` | `proposal-validated` | 这份 Preview 协议结构过不过 | 过 | 过：`validation record`（外部套件）、业务测试 | 过：`{reportId, outcome:PASSED}`。help「校验 Preview 的协议结构」 | 过 | 过：不跑业务套件 | 进阶 | 成立 |
| `governance validation record` | `validation-recorded` | 把外部套件已给出的结果绑上去 | 过 | 过：`preview validate`（KC 自己跑） | 过：`{reportId, outcome:PASSED}`。help「记录外部套件结果」 | 过 | 过：不执行检查 | 进阶 | 成立 |
| `governance proposal merge` | `proposal-merged` | 清单齐则快进仓 Ref | 过 | 过：`writer commit`、`git merge` | 过：`{proposalId, commitId}`；随后 `read` 见 proposed。help「合入 Proposal」 | 过 | 过：construct 只 merge | 进阶 | 成立 |

---

## 7. 投影、出站、墙外观察

场景：`projection-synced`、`observation-refreshed`、`repository-attached` hooks、`proposal-opened` gates。
...
| `operations projection notice` | `observation-refreshed` | 告诉投影墙外观察变了 | 过 | 过：`sync`（索引追 HEAD）、`writer head`（仓版本） | 过：`{repository, basisCommit, revision}`；随后 HEAD commit 仍在。缺 `--repo` → `USAGE_INVALID`。help「通知 Bound State 观察变化，不推进仓 HEAD」 | 过：句柄用 `--object`/`--aspect` 写在用法第二行 | 过：HEAD 不变 | 进阶 | 成立 |
...
| `operations gate add` | `proposal-opened` | 合入必须备齐哪些证据 | 过 | 过：`hook add`、`preview validate` | 过：`{id, on:merge}`。help「登记 Gate」 | 过 | 过 | 进阶 | 成立 |
| `operations gate list` | 同上 | 现在有哪些 gate | 过 | 过：`hook list`（`bindings` vs `rules`） | 过：`{rules}`。help「列出 Gate」 | 过 | 过 | 进阶 | 成立 |
| `operations gate remove` | 同上 | 去掉这个 gate | 过 | 过：`hook remove` | 过：`{revoked}`。help「移除 Gate」 | 过 | 过 | 进阶 | 成立 |
...
场景：`http-served`、`access-audited`、`knowledge-set-defined` probe、`retrieval-refined`。

| 命令 | 场景 | 一句语义 | 问 | 刀（对手） | 对 | 形 | 侧 | 路 | 成立 |
|---|---|---|---|---|---|---|---|---|---|
| `operations audit access` | `http-served` | 最近发生了哪些访问 | 过 | 过：`catalog audit`（登记表）、`log`（对象史） | 过：`{source:access, entries?}`；空窗不是错误。help「查看访问账」 | 过 | 过 | 进阶 | 成立 |
| `operations audit hitmap` | `http-served` | 检索命中了哪些对象 | 过 | 过：`audit access`（事件 vs 对象） | 过：`{source:hitmap, hits}`。help「按对象汇总检索命中，不是访问事件账」 | 过 | 过 | 进阶 | 成立 |
| `operations audit trace` | `access-audited` / usage probe | 这一次请求的证据链 | 过 | 过：`audit access`（分页账）、`hitmap` | 过：缺 `--trace-id` → `USAGE_INVALID`；成功 `{traceId, entries[]}`（kind=access/feedback/refine/retrieval）。help「查看请求追踪」 | 过 | 过 | 进阶 | 成立 |
| `operations feedback record` | `knowledge-set-defined` / `retrieval-refined` | 给这次检索记一条人的对错 | 过 | 过：`preview validate` 的 PASSED/FAILED | 过：非法 outcome → `USAGE_INVALID`；成功 `{traceId, outcome, recorded:true, …}`。help「记录检索反馈」 | 过 | 过：晚于请求的独立调用 | 进阶 | 成立 |

---

## 9. Catalog 进阶与部署

场景：`catalog-initialized`、`catalog-audit-granted`、`catalog-archived`、`deployment-restored`、`http-served`。

| 命令 | 场景 | 一句语义 | 问 | 刀（对手） | 对 | 形 | 侧 | 路 | 成立 |
|---|---|---|---|---|---|---|---|---|---|
| `catalog audit` | `catalog-initialized` / `catalog-audit-granted` | 这间 Catalog 的登记表怎么变过来的 | 过 | 过：`log`（对象修订）、`audit access` | 过：`{source:catalog, catalogId, entries}`。无 `catalog.audit.read` → 拒绝。help「查看 Catalog 登记表历史」 | 过 | 过：不放行库存发现 | 进阶 | 成立 |
| `catalog archive` | `catalog-archived` | 结束这间 Catalog 的生命周期 | 过 | 过：`detach`（一个仓）、删部署 | 过：`{catalog, archived:true}`，随后 `show.archived=true`。无管理权 → 拒绝。help「归档当前 Catalog」 | 过 | 过 | 进阶 | 成立 |
| `deployment init` | `catalog-initialized` fixture / probe | 按配置文件把部署建起来 | 过 | 过：手建目录、`serve` | 过：成功 `{initialized:true, catalogs}`；无 `--config` → `USAGE_INVALID`。help「初始化部署」 | 过 | 过：不猜本机 Home | 部署 | 成立 |
| `deployment status` | 同上 / `deployment-restored` | 既有部署恢复好了没有 | 过 | 过：`show`、`writer head` | 过：`{status:ready, catalogs:[{id,head,archived,repositories}]}`；无 `--config` → `USAGE_INVALID`。help「查看部署恢复状态」 | 过 | 过：缺耐久状态失败关闭 | 部署 | 成立 |
| `deployment system publish` | `system-schema-published` 的部署口 | 把 System Schema 写入绑定仓 | 过 | 过：`writer put` 到 `kr://kc/system`（禁止） | 过：`{repositoryId, commit, metaSchema, metaSchemaDigest, seeded}`；无 `--config` → `USAGE_INVALID`。help「把 System Schema 写入绑定仓」 | 过 | 过：不登记业务 Snapshot | 部署 | 成立 |
| `deployment identity migrate` | 部署管理 go-test | 把历史身份绑定迁到核验过的主体 | 过 | 过：`grant add` 重发、改用户名 | 过：`{status:APPLIED\|REPLAYED, principal, migratedRules}`。help「迁移历史身份绑定」 | 过 | 过：重放不重建已撤销规则 | 部署 | 成立 |
...
场景：`file-view-planned` → `dataset-mounted`。不在 `cliSurface`；`TestSceneCatalogCoversPublicProductSurfaces` 另钉。

| 命令 | 场景 | 一句语义 | 问 | 刀（对手） | 对 | 形 | 侧 | 路 | 成立 |
|---|---|---|---|---|---|---|---|---|---|
| `kcfs plan` | `file-view-planned` | 这份 Dataset 会挂成哪些只读路径 | 过 | 过：`dataset define`（坐标）、`kcfs mount`（FUSE）、扫描仓伪造工作树 | 过：打印 mount manifest。help 第一句「kcfs mounts a Knowledge Catalog Dataset」 | 过：必须 `--dataset` + `--root` | 过 | 进阶 | 成立 |
| `kcfs mount` | `dataset-mounted` | 把这份 Dataset 挂进现有项目当只读目录 | 过 | 过：`plan`、Linux `mount`、checkout | 过：同一份 manifest，然后服务到信号。Linux FUSE 才是真实挂载 | 过 | 过：无 `--dataset` 不挂；知识仓无 mount 不得扫描 | 进阶 | 成立 |
...
| `create` | `managed-repository-created` | 是（两种模式） |
| `attach` | `repository-attached` | 是 |
| `detach` | `repository-archived` | 是 |
| `grant add` | 各 `*-granted` | 是 |
| `grant list` | `catalog-initialized` | 是 |
| `grant remove` | `grant-revoked` | 是 |
| `schema list` | `schema-read-granted` | 是 |
| `schema describe` | `schema-read-granted` | 是 |
| `search` | `knowledge-search-granted` | 是 |
| `read` | `knowledge-read-granted` | 是 |
| `resolve` | `knowledge-published` | 是 |
| `relations` | `knowledge-published` | 是 |
| `provenance` | `knowledge-published` | 是 |
| `log` | `knowledge-published` | 是 |
| `binding show` | `access-handle-published` | 是 |
| `access` | `access-handle-published` | 是 |
| `invoke` | `access-handle-published` | 是 |
...
| `dataset retire` | `dataset-retired` | 是 |
| `dataset overlay` | `file-view-planned` | 是 |
| `governance proposal create` | `proposal-opened` | 是 |
| `governance preview create` | `proposal-previewed` | 是 |
| `governance preview validate` | `proposal-validated` | 是 |
| `governance validation record` | `validation-recorded` | 是 |
| `governance proposal merge` | `proposal-merged` | 是 |
| `operations projection sync` | `projection-synced` | 是 |
| `operations projection describe` | `projection-synced` | 是 |
| `operations projection notice` | `observation-refreshed` | 是 |
...
| `operations gate add` | `proposal-opened` | 是 |
| `operations gate list` | `proposal-opened` | 是 |
| `operations gate remove` | `proposal-opened` | 是 |
| `operations audit access` | `http-served` | 是 |
| `operations audit hitmap` | `http-served` | 是 |
| `operations audit trace` | `access-audited` | 是 |
| `operations feedback record` | `retrieval-refined` | 是 |
| `catalog audit` | `catalog-audit-granted` | 是 |
| `catalog archive` | `catalog-archived` | 是 |
| `deployment init` | `catalog-initialized` | 是 |
| `deployment status` | `deployment-restored` | 是 |
| `deployment system publish` | `system-schema-published` | 是 |
| `deployment identity migrate` | 部署 go-test | 是 |
| `serve` | `http-served` | 是 |
| `kcfs plan` | `file-view-planned` | 是 |
| `kcfs mount` | `dataset-mounted` | 是 |
...
`absent-product-surfaces` 已钉：MCP、对象 LIST、checkout/export、connector-run、APPEND/Stream 正路径都不存在。
