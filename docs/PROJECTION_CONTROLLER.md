...
Repository HEAD 对账与 Dataset 服务版是不同生命周期。Dataset 的知识发布由应用层先冻结候选快照、准备部署所提供的必要查询投影，再原子推进 Catalog 当前发布记录。失败不替换上一服务版，也不回滚源仓提交。纯文件 Dataset 和未配置检索的部署不因此取得 SEARCH 能力。

服务版投影以 Repository 与固定快照共享，不按 Dataset revision 复制。源 HEAD 前进不能使仍在服务的固定快照失去索引；消费优先使用保留的固定投影。应用装配提供当前发布快照的清单，独立恢复通道在启动与周期对账时从 Catalog 重建这些派生能力。消费请求只检查就绪状态，不承担恢复。动态 State 仍保留独立 observation basis，不把瞬时值冻结为 Dataset 文件。
