...
Relation 是独立维护的知识对象，拥有自己的存储仓、版本与权限。端点使用带 Repository 身份的 KnowledgeRef，可以引用其他仓的对象；关系存储仓不必包含端点对象。

写关系只提交到关系所在仓。Writer 校验关系形状，但不访问端点仓、不要求端点已接入或当前存在，也不修改端点对象；跨仓引用不形成跨仓事务。引用本身不授予端点对象的读权，不使其自动加入 Dataset，且不改变 `schema_ref` 的同仓解析规则。

一跳查询在当前 Dataset 的固定成员范围内查找关系存储仓的投影，不能只查询端点所在仓。每个候选按其存储仓的固定 commit 回读，文件范围在正文回读前检查；端点的引用身份不替代关系的存储坐标。后续读取端点仍按同一 Dataset 的范围、版本与当前权限执行。缺少必要投影应明确报告，不回退为 authority 扫描。

图访问分两层：一跳关系查询与有界类型化遍历。遍历在请求开始时固定范围（Dataset 清单或单仓版本 pin），沿类型化关系步骤（关系类型、端点角色与方向）扩展到显式跳数上界，返回按对象去重的到达集合与全部入选边，逐边保留其存储仓与 commit；同一对象由多条边到达时按合同显式去重并披露入选边，不枚举全部路径，也不承诺最短路径。授权前置到扩展动作：边只来自其存储仓所在的已授权范围，从某对象继续扩展要求该对象所在仓在本次固定范围内，越界 frontier 止步并显式标记，不借遍历扩大范围或披露范围外拓扑。遍历共享请求级预算，超限显式标记不完整并给出可续位置，循环按节点去重。无界遍历与任意图查询语言仍被拒绝；图引擎只能作为类型化请求的编译目标执行选择（派生、可重建），不是第二权威。

### 7.6 统一访问的操作族与完整性分级

统一访问层组合每次消费的固定范围、授权视图与预算；它不新增动态 Store，也不把各操作压成一种查询语言。公开访问按操作族组织，每族有类型化请求、独立结果合同与自己的完整性等级：

- 发现：`DISCOVER` / `BROWSE` / `DESCRIBE_SCHEMA`，有界库存与已发布实体名单；
- 召回检索：`SEARCH`（词法为现役默认；语义与混合召回按另行合同扩展，见 [检索设计](RETRIEVAL.md)）与 `RERANK`（只评判输入候选，不承担召回）；
- 精确读取：`RESOLVE` / `READ` / `READ_ADDRESS`，权威正文；
- 关系访问：`RELATIONS` 一跳查询与 `TRAVERSE` 有界邻域遍历（本文 §7.5）；
- 来源与历史：`GET_PROVENANCE` / `LOG` / `DIFF`；
- 外部资源：`RESOLVE_BINDING` / `ACCESS` / `INVOKE`，保留 observation basis。

结果信封必须能区分四级完整性：可证穷尽（范围内证明无遗漏）、近似窗口（排名候选，不承诺穷尽）、有界截断（超预算显式标记不完整并给出可续位置）、观察依据（只对 observation basis 负责）。近似与截断都必须披露依据，不能伪装成穷尽。
...
图访问分两层——一跳关系查询与有界类型化遍历；关系允许跨仓引用，写入仍限于关系所在仓；不建通用图查询语言，图引擎只能作为类型化请求的编译目标执行选择。正文：§7.5。
...
语义召回（semantic / hybrid）是 SEARCH 之上的另行合同扩展：召回策略是请求级闭集，候选资格先于召回，semantic 与 hybrid 结果一律近似并披露依据；向量投影是派生可重建投影，不写成第四个字段访问声明。正文：[检索设计](RETRIEVAL.md) §8。

#### ADR-029

统一访问按操作族组织，每族声明完整性等级（可证穷尽 / 近似窗口 / 有界截断 / 观察依据），结果信封分别说明投影覆盖、完整性与观察时效。正文：§7.6。

### 9.3 核心不变量（K-01..K-28）

本节保留设计推导层的语义结论；规范性的可证伪属性、禁止观察和自动化证据统一登记在
[`ARCHITECTURE_INVARIANTS.md`](reviewed/architecture-invariants.md)（与 ADR / MVP 的交叉见该文 §2.1）。
