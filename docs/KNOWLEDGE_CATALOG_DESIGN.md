...
一跳查询在当前 Dataset 的固定成员范围内查找关系存储仓的投影，不能只查询端点所在仓。每个候选按其存储仓的固定 commit 回读，文件范围在正文回读前检查；端点的引用身份不替代关系的存储坐标。后续读取端点仍按同一 Dataset 的范围、版本与当前权限执行。缺少必要投影应明确报告，不回退为 authority 扫描，也不扩展为递归图遍历。
...
图核心最多保证一跳；关系允许跨仓引用，写入仍限于关系所在仓，不建通用图语言。正文：§7.5。
