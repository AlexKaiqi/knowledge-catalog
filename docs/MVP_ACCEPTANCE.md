...
| M1 | 接入 Repository | 新建 FileGit；挂已有普通 Git；clone `--link`；挂真实 Gitea；挂 Dolt；均保留独立版本和生命周期 | 不污染 `--dir` 源仓；Catalog/动态运行时/MySQL/缓存不能冒充 Repo；不支持的能力显式失败 |
| M2 | 组织 Workspace | 根 mount、嵌套 mount、只读和可写成员组成唯一树；配方可随 Git 分享；一次解析得到固定 `{Repo->commit}` | 不复制成员内容；路径不能重叠或无归属；同仓不能重复；配方不发权 |
...
| M8 | 更新感知与复现 | Agent 任务在 P0/V1 开始；上游推进 V2 后本任务仍可重放 P0；新任务解析 P1 并看到 V2 | 会话中途不能偷偷跟 HEAD；新版本不能改写引用方 Repo；动态 observation 复现属于上层产品验收 |
...
| M2 | 建根、嵌套、只读、可写 mount；resolve 保存各仓 commit；重叠路径、无归属路径、旧 `view` flag/verb 都必须失败 |
...
| M8 | J4 让旧 Agent 跨外部更新保持 V1、新 Agent 看到 V2；动态 State/Stream 的 cut 与重放改由上层产品单独验收 |
...
`LIVE_MATERIALIZATION.md` 第 5 节是索引 MVP 的权威逻辑契约。仓库根 M7 至少用一个
Snapshot reference provider 全量验证：

- `MATCH(AllTerms/AnyTerms/Phrase)`，bare MATCH 与明确 FieldRef；
- typed `EQ/IN/NEQ/EXISTS/MISSING`、number/time range、string `PREFIX`；
- clause 隐式 AND、IN 表达同字段 OR、一个显式 SORT、LIMIT 与 opaque continuation；
- AccessHint/类型/字段歧义拒绝，不能回退整包 JSON contains；
- provider `Exact/Superset/Approximate/Unsupported`，coverage/freshness 与 complete/partial；
- Candidate 不返回正文，公开 hit 在请求 pin hydrate 完整 KnowledgeValue、版本和 evidence；
- schema/access、physical revision、basis divergence 分别触发正确的 Apply 或 Rebuild；
- 无权成员不向 provider 发请求，也不通过 hit、计数或错误差异泄漏。

其它 provider 可以只实现子集，但必须由 `Probe` 如实拒绝；标称支持 `search-mvp` 的 profile
必须完整通过同一矩阵。动态 State 的 invalidate/pull、Serving State、checkpoint/reconcile 和
observation basis 属于上层 Materialization/Retrieval 产品验收，不进入仓库根 M0-M13 阻塞项。
Stream、任意布尔 AST、CONTAINS/GLOB/REGEX、Facet、语义/向量、聚合与 Join 是明确非目标。
