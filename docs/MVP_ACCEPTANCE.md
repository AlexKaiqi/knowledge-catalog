...
| V3 | 同仓多子树 | 同一 Repository 可用相同 selector/baseRev 投影多个互不重叠的 `SubPath`；pin 中仍只有一个 commit |
...
| V7 | 授权 | 先检查 `read-workspace`，再逐 Repository 检查 `read`；无权成员不出现在 plan 或 mount 中 |
...
同一 Repository 的不同子树），验证 `cat`、`rg`、只读拒绝、pin 和卸载清理。非 Linux 环境明确
...
`expectedTargetCommit`、`command_id`、逐 Repository 结果和冲突恢复。不能把 FUSE 的普通 write
系统调用直接解释成协议 COMMIT，也不能伪装跨 Repository 原子事务。
