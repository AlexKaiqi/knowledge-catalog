...
输出 `manifest.json`、`model.json`、`bootstrap.ndjson`、`events.ndjson`。生成器流式写文件，S3 不把百万张表放进内存。load runner 应把每条 table family 翻译成现有 ChangeSet，再通过公开 Writer API 提交；结果证据遵循 `docs/SCALE_BENCHMARK.md`。
