...
Feature: probe workspace cli
  ...
  Scenario: consume discovers, pins, searches, then reads
    ...
    Then the output has:
      ...
      | pinId       | nonempty |
      | out         | nonempty |
    When I run `kc knowledge search --as agent:copilot --pin $pinFile --query merchandise --server $server`
    Then 1 hit metric/gmv with full canonical
    When I run `kc knowledge read --as agent:copilot --pin $pinFile --object metric/gmv --aspect definition --server $server`
    Then the output includes:
      | [].knowledgeRef.object | metric/gmv |
      | [].value.name          | Gross merchandise value |
    When I run `kc knowledge resolve --as agent:copilot --pin $pinFile --object metric/gmv --server $server`
    Then the output includes:
      | [].status | RESOLVED |
