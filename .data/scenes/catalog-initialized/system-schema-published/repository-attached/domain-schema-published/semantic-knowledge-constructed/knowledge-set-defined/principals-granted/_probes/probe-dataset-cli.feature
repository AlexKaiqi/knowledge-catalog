# 在 principals-granted 上：help consume 最短路径是发现 → SEARCH/READ --dataset。不是 --repo 维护读。

Feature: probe workspace cli

  Scenario: consume discovers, searches, then reads
    ...
    When I run `kc relations --as agent:copilot --dataset scene-set --object metric/gmv --server $server`
    Then the output includes:
