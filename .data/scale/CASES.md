...
执行任何用例前还必须通过 [`ENVIRONMENT.md`](ENVIRONMENT.md) 的对应档位环境门禁。
服务能够启动或功能 smoke 通过，不等于环境具备压测资格。

压测不属于 `.data/scenes` 功能验收，也不由 `make test` 执行。功能验收回答行为
...
- 执行压测前，目标 revision 必须已通过 `make test`；结果只作为前置证据，不计入压测耗时。
...
- 业务模型、事件流和负载配置位于本目录；原始输出写入
  `.data/scale/runs/<run-id>/`，均不提交。
- `make test` 不运行本清单。未来 load runner 应提供单用例入口，例如
