...
describe("T4 — Command Idempotency", () => {
  it("replays an identical command_id with REPLAYED; conflicts on different payload", () => {
    const { writer, repositoryId, rootCommitId } = setup();
    ...
    const r1 = writer.commit("cmd-1", cs);
    const r2 = writer.commit("cmd-1", cs);
    ...
    expectCode(() => writer.commit("cmd-1", different), "IDEMPOTENCY_CONFLICT");
  });
});
