...
// SERVICE_ARCHITECTURE §§3.4, 3.6, 11.1 and WS-02: the file gateway must
...
func declaredFileGateway(t *testing.T) (apphome.DeploymentConfig, string) {
	...
	if raw, err := exec.Command("git", "init", "--bare", remote).CombinedOutput(); err != nil {
		t.Fatalf("bare Git: %s %v", raw, err)
	}
	cfg := apphome.DeploymentConfig{Version: 1, StateDir: filepath.Join(root, "durable"), CacheDir: filepath.Join(root, "cache"), Auth: "local", BootstrapPrincipal: "agent:operator", Catalogs: []apphome.CatalogBinding{{ID: "kr://files/catalog", Remote: remote}}}
	if err := apphome.InitializeDeployment(cfg, func(dir, principal string) error {
		return WriteAllow(dir, AllowFile{Rules: []AllowRule{
			...
			{ID: "file-reader", Principal: "agent:reader", Actions: []string{"workspace.resolve"}, Catalog: cfg.Catalogs[0].ID, Workspace: "protocol"},
		}})
	}); err != nil {
		t.Fatal(err)
	}
	...
	if _, err := cat.DefineWorkspace("protocol", 1, []catalog.WorkspaceSource{{Repository: knowledge.SystemRepositoryID, Selector: snapshot.DefaultRef}}); err != nil {
		t.Fatal(err)
	}
...
func TestWorkspaceFileGatewayRestoresDeclaredDeployment(t *testing.T) {
	...
	var fixedPin catalog.ResolvedWorkspace
	for _, explicit := range []bool{false, true} {
		...
		t.Run(name, func(t *testing.T) {
			coordinate := map[string]any{"workspace": "protocol", "view": "semantic"}
			if explicit {
				coordinate["catalog"] = cfg.Catalogs[0].ID
			}
			mountResponse := fileGatewayRequest(t, handler, "agent:reader", "/workspace-files/v1/mounts:list", coordinate)
			...
			listing := fileGatewayRequest(t, handler, "agent:reader", "/workspace-files/v1/tree:list", coordinate)
			...
			read := fileGatewayRequest(t, handler, "agent:reader", "/workspace-files/v1/file:read", coordinate)
	...
	denied := fileGatewayRequest(t, handler, "agent:reader", "/workspace-files/v1/file:read", map[string]any{
		"workspace": "protocol", "view": "semantic", "pin": fixedPin, "mountPath": "knowledge/system", "file": "_meta/repository.yaml",
	})
...
func TestWorkspaceFileGatewayBorrowsHomeUntilConcurrentReadsFinish(t *testing.T) {
	...
	coordinate := workspaceFileCoordinate{Workspace: "protocol", View: "semantic"}
	request := func() *http.Request {
		r := httptest.NewRequest(http.MethodPost, "/workspace-files/v1/mounts:list", nil)
