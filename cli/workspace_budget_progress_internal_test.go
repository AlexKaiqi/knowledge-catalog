...
func workspaceBudgetFixture(t *testing.T, members, objects int, superset bool) (map[kernel.RepositoryID]*workspaceBudgetEngine, func(index.SearchBudget, string) (retrieval.SearchResult, error)) {
	...
	for member := 0; member < members; member++ {
		...
		sources = append(sources, catalog.WorkspaceSource{Repository: id, Selector: snapshot.DefaultRef})
	}
	...
	cat.LoadState(catalog.CatalogState{CatalogID: "kr://acme/catalog", Repositories: repositories, Workspaces: []catalog.WorkspaceDefinition{{WorkspaceID: "budget", Revision: 1, Sources: sources}}})
	dir := t.TempDir()
	...
	cx := &invocation{Home: dir, WS: ws, Flags: map[string]FlagValue{"workspace": "budget", "as": "agent:budget"}}
