...
func TestCatalogViewsChecksAndKnowledgeResolve(t *testing.T) {
	...
	workspaces := catalogView["workspaces"].([]any)
	if len(workspaces) != 2 {
	...
	expectCode(t, kc(home, "workspace", "show", "--workspace", "missing"), "USAGE_INVALID")
	...
	accessPlan := asMap(t, body(t, kc(home, "operations", "access-spec", "describe", "--pin", pinFile)))
	...
	resource := asMap(t, body(t, kc(home, "knowledge", "access", "--pin", pinFile,
	...
	direct := asMap(t, body(t, kc(home, "knowledge", "invoke", "--pin", pinFile,
	...
	resolved := body(t, kc(home, "knowledge", "resolve", "--pin", pinFile, "--object", "policy/coverage")).([]any)
	...
	aspectResolved := body(t, kc(home, "knowledge", "resolve", "--pin", pinFile,
	...
	expectCode(t, kc(home, "knowledge", "resolve", "--pin", pinFile,
	...
	absent := body(t, kc(home, "knowledge", "resolve", "--pin", pinFile, "--object", "missing/coverage")).([]any)
	if len(absent) != 0 {
		t.Fatalf("workspace resolve of a missing object is an empty union: %#v", absent)
	}

	history := asMap(t, body(t, kc(home, "knowledge", "log", "--pin", pinFile, "--object", "policy/coverage", "--limit", "1")))
	if history["exhausted"] != nil || len(history["logs"].([]any)) != 1 {
		t.Fatalf("knowledge log must return a bounded page: %#v", history)
	}
	zeroLog := asMap(t, body(t, kc(home, "knowledge", "log", "--pin", pinFile, "--object", "policy/coverage", "--limit", "0")))
	if zeroLog["exhausted"] != nil || len(zeroLog["logs"].([]any)) != 1 {
		t.Fatalf("knowledge log --limit 0 must mean the default page: %#v", zeroLog)
	}
	expectCode(t, kc(home, "knowledge", "log", "--pin", pinFile, "--object", "policy/coverage",
		"--continuation", "not-a-cursor"), "USAGE_INVALID")
	expectCode(t, kc(home, "knowledge", "log", "--pin", pinFile, "--object", "policy/coverage",
