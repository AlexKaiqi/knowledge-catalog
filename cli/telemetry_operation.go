...
func newOperationTelemetry(ctx context.Context, runtime *telemetry.Runtime, operation string, flags map[string]FlagValue) *operationTelemetry {
	observation := &operationTelemetry{runtime: runtime, ctx: ctx}
	if runtime == nil {
		return observation
	}
	...
	observation.evidence = func(kind, outcome string, elapsed time.Duration, bytes int64) {
		runtime.RecordEvidence(ctx, kind, outcome, elapsed, bytes)
	}
	return observation
}
