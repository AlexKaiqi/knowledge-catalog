...
func TestUnknownFlagIsRejectedBeforeOpeningHome(t *testing.T) {
	result := invokeInternal("knowledge-read", map[string]FlagValue{"home": t.TempDir(), "wrokspace": "agent"})
...
func TestTransportSpecificFlagsCannotLeakAcrossSurfaces(t *testing.T) {
	result := invokeInternal("knowledge-read", map[string]FlagValue{"home": t.TempDir(), "listen": "127.0.0.1:0"})
