...
func AllowsOp(field AccessField, op SearchOp) bool {
	switch op {
	case OpMatch:
		return field.Has(HintText)
	case OpEQ, OpIN, OpNEQ, OpExists, OpMissing:
		return field.Has(HintFilter)
	case OpPrefix:
		return field.Has(HintFilter) && StringType(field.Type)
	case OpGT, OpGTE, OpLT, OpLTE:
		return field.Has(HintFilter) && RangeType(field.Type)
	case OpSort:
