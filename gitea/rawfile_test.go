...
func TestGiteaRawFileStoreRoundTrip(t *testing.T) {
	...
	_, err = raw.ApplyRawCommit(repository.RawFileChangeSet{
		TargetRepository: id, TargetRef: "refs/heads/main",
		BaseCommit: root, ExpectedTargetCommit: root,
		Changes: []repository.RawFileChange{{Path: "vfs/other.md", Content: []byte("x")}},
	})
	testkit.ExpectCode(t, err, kernel.ErrNonFastForward)

	// Remove deletes the path.
	removed, err := raw.ApplyRawCommit(repository.RawFileChangeSet{
		TargetRepository: id, TargetRef: "refs/heads/main",
		BaseCommit: commit, ExpectedTargetCommit: commit,
		Changes: []repository.RawFileChange{{Path: "vfs/note.md", Remove: true}},
	})
	if err != nil {
		t.Fatal(err)
	}
