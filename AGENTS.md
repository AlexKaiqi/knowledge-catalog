...
- 协议/契约/Ingress/Repository/View/T1–T12 不够 → 改 **仓库根** 并提交，然后把 main **合入**场景（见下）。
...
- **Schema 是知识**，不是项目源码。正式形态是知识 Repository 里的 `schema/*` 对象（Ingress COMMIT，可 RESOLVE/READ/ORIGIN）。未入库前只能临时放 `.data/`，禁止 `schemas/`、`src/schemas/` 或任何会进 git 的路径。
...
src/api/           Ingress / Access / ingest+reconcile / Refine
src/control-plane/ PROPOSAL → Preview → Validate → Merge
src/catalog/       ViewGeneration、联邦读、Promote
...
- 不要把 schema 写成项目文件。Schema 是知识对象，走 Ingress；草稿只放 `.data/`。
- 不要把 `.scenes/` 提交进 git，也不要写进 `.cursorignore`。
- 不要为场景新增 Write Surface。采集输出仍是 ChangeSet 预览，经 Ingress `commit` / `append`。
...
- Access：`READ(ref)` 拼装（可 `AspectSelector`）；`readAddress` 读单单元。Projection 用同一 selector 编 FTS；`permissions` 等 ACL 投影不进索引。见 `ASPECT_ACCESS.md`。
...
- `COMMIT` 推 Ref；`promote` 只动 Channel。Agent 读的是 Channel 钉住的 Generation，不是 `main`。
- Catalog namespace、WriteBinding、跨进程幂等、HTTP/MCP 网关尚未实现。缺这些先问归属，再决定补 main 还是场景。
...
- `KNOWLEDGE_CATALOG_DESIGN.md` — 设计与 K-01..K-24
- `ASPECT_ACCESS.md` — Aspect 读/检索业界对照与决策
- `WALKTHROUGH_v5.1.md` — 端到端推演与产品缺口
