// Package datasetfs projects one resolved Catalog knowledge set into host
...
// File is one immutable file in a resolved knowledge set mount. Path is relative
...
func (p Plan) Validate() ([]Target, error) {
	...
	if len(p.Mounts) == 0 {
		return nil, fmt.Errorf("workspace %s has no visible mounts", p.SetID)
	}
