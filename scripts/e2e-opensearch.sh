...
for _ in {1..45}; do
  if curl -fsS "$endpoint/_cluster/health" >/dev/null 2>&1; then
    KC_TEST_OPENSEARCH_URL="$endpoint" "${GO:-go}" test -json -count=1 -v ./retrieval/opensearch
    KC_TEST_OPENSEARCH_URL="$endpoint" "${GO:-go}" test -json -count=1 -run '^TestLiveOpenSearchStateProjectionRefreshAndSameBasisHydrate$' -v ./index
